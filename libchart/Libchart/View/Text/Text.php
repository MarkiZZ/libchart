...
class Text
{
    ...
    public function getWidth($text, $fontFileName)
    {
        $fontSize = 8;
        $lineSpacing = 1;

        list ($llx, $lly, $lrx, $lry, $urx, $ury, $ulx, $uly) = imageftbbox($fontSize, 0, $fontFileName, $text, ["linespacing" => $lineSpacing]);
    ...
    public function printDiagonal($img, $px, $py, $color, $text)
    {
        ...
        list ($lx, $ly, $rx, $ry) = imageftbbox($fontSize, 0, $fontFileName, $text, ["linespacing" => $lineSpacing]);
        $textWidth = $rx - $lx;
