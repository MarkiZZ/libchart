 ...
 */
class HorizontalBarChart extends \Libchart\View\Chart\BarChart
{
    /**
    ...
    public function __construct($width = 600, $height = 250)
    {
        ...
        $this->plot->setGraphPadding(new \Libchart\View\Primitive\Padding(5, 30, 30, 50));
    }

    /**
     * Computes the layout.
     */
    protected function computeLayout($caption)
    {
        if ($this->hasSeveralSerie) {
            $this->plot->setHasCaption(true);
            $this->plot->setDesiredCaptionWidth($caption->getDesiredWidth());
        }
    ...
    protected function printAxis()
    {
        ...
        for ($value = $minValue; $value <= $maxValue; $value += $stepValue) {
            ...
            // Background grid
            imagerectangle($img, $x - 1, $graphArea->y1, $x, $graphArea->y2, $palette->axisColor[0]->getColor($img));
        }
        ...
        for ($i = 0; $i <= $pointCount; $i++) {
            ...
            imagerectangle($img, $graphArea - 3, $y, $graphArea - 2, $y + 1, $palette->axisColor[0]->getColor($img));
            imagerectangle($img, $graphArea - 1, $y, $graphArea, $y + 1, $palette->axisColor[1]->getColor($img));
            ...
            // Background grid
            imagerectangle($img, $graphArea->x1, $y, $graphArea->x2, $y + 1, $palette->axisColor[0]->getColor($img));
        }
    }
    ...
    protected function getAxisLabelWidth()
    {
        $text = $this->plot->getText();
        $pointList = $this->getFirstSerieOfList();
        $pointCount = count($pointList);
        ...
        for ($i = 0; $i < $pointCount; $i++) {
            $label = $pointList[$i]->getX();
            $currentWidth = $text->getWidth($label, $text->fontCondensed);
    ...
    protected function getDesiredHeight()
    {
        $pointList = $this->getFirstSerieOfList();
        $pointCount = count($pointList);

        return 60 * $pointCount;
    }
    ...
    public function render($fileName = null)
    {
        ...
        $this->plot->setGraphPadding(new \Libchart\View\Primitive\Padding(5, 30, 30, 30 + $this->getAxisLabelWidth()));
        ...
        $desiredHeight = $this->getDesiredHeight();
        $givenArea = $this->plot->getGraphArea();
        $givenHeight = $givenArea->y2 - $givenArea->y1;
        $this->plot->setDimensions($this->plot->getWidth(), $this->plot->getHeight() + $desiredHeight - $givenHeight);
        $this->computeLayout($caption);
