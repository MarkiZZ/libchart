 ...
 */
abstract class BarChart extends \Libchart\View\Chart\Chart
{
    protected $bound;
    ...
    protected function __construct($width, $height)
    {
        ...
        $this->bound = new \Libchart\View\Axis\Bound();
    ...
    {
        $this->axis = new \Libchart\View\Axis\Axis($this->bound->getYMinValue(), $this->bound->getYMaxValue());
     ...
     * Create the image.
     */
    protected function createImage($axis = true)
    {
        parent::createImage();
    ...
    {
        if ($this->dataSet instanceof \Libchart\Model\XYDataSet) {
            $pointList = $this->dataSet->getPointList();
            $pointCount = count($pointList);

            return $pointCount < $minNumberOfPoint;
        } elseif ($this->dataSet instanceof \Libchart\Model\XYSeriesDataSet) {
    ...
    protected function getDataAsSerieList()
    {
        ...
        $serieList = null;
        if ($this->dataSet instanceof \Libchart\Model\XYSeriesDataSet) {
            $serieList = $this->dataSet->getSerieList();
        } elseif ($this->dataSet instanceof \Libchart\Model\XYDataSet) {
            $serieList = [];
